...
let sto_path = store /[A-Za-z0-9_.-\/]+/
(* View: http_get_url_field *)
let http_get_url_field = field "status_code" sto_num
                    | field "path" sto_path
                    | field "digest" sto_word

(* View: http_get_field *)
let http_get_field = field "connect_timeout" sto_num
                    | field "connect_port" sto_num
                    | field "nb_get_retry" sto_num
                    | field "delay_before_retry" sto_num
                    | block "url" http_get_url_field

(* View: real_server_field *)
let real_server_field = field "weight" sto_num
                      | block "TCP_CHECK" tcp_check_field
                      | block "HTTP_GET" http_get_field

(* View: virtual_server_field *)
let virtual_server_field = field "delay_loop" sto_num
                         | field "lb_algo" sto_word
                         | field "lb_kind" sto_word
                         | field "persistence_timeout" sto_num
